...
// add attributes price to price
      if ($product_check->fields['products_priced_by_attribute'] == '1' and $product_att_query->RecordCount() >= 1) {
        ...
        $the_retail_net_price = $retail_net_price + $the_base_price;
      } else {
        $the_retail_net_price = $retail_net_price;
      }
...
// Specials and Tax Included
 function zen_get_products_display_retail_net_price($products_id) {
  global $db, $currencies, $currencies_no;
...
// show case only
  ...
  $show_display_price = '';
  $display_normal_price = zen_get_products_retail_net_price($products_id);
  $display_special_price = zen_get_products_special_price($products_id, true);
  $display_sale_price = zen_get_products_special_price($products_id, false);
...
//echo 'I SEE 2 for ' . $products_query->fields['products_discount_type'] . ' - ' . $products_query->fields['products_discount_type_from'] . ' - '. $check_amount . ' new: ' . $discounted_price . ' qty: ' . $check_qty;
                ...
                if ($_SESSION['customer_whole'] && $_SESSION['customer_whole'] != '0' ) {
                    $discounted_price = $display_price - ($display_price * ($products_discounts_query->fields['discount_price_w']/100));
                } else {
            ...
            if (!$display_specials_price) {
              ...
              if ($check_amount != 0) {
                ...
                if ($_SESSION['customer_whole'] && $_SESSION['customer_whole'] != '0' ) {
                    $discounted_price = $display_specials_price - ($display_specials_price * ($products_discounts_query->fields['discount_price_w']/100));
                } else {
        ...
        case '2':
          if ($products_query->fields['products_discount_type_from'] == '0') {
            if ($_SESSION['customer_whole'] && $_SESSION['customer_whole'] != '0' ) {
                $discounted_price = $products_discounts_query->fields['discount_price_w'];
           } else {
        ...
        case '3':
          if ($products_query->fields['products_discount_type_from'] == '0') {
            if ($_SESSION['customer_whole'] && $_SESSION['customer_whole'] != '0' ) {
                $discounted_price = $display_price - $products_discounts_query->fields['discount_price_w'];
            } else {
            ...
            if (!$display_specials_price) {
                ...
                if ($_SESSION['customer_whole'] && $_SESSION['customer_whole'] != '0' ) {
                    $discounted_price = $display_specials_price - $products_discounts_query->fields['discount_price_w'];
                } else {
