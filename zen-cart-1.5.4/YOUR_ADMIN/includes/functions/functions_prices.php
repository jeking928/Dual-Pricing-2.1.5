...
// computes products_price + option groups lowest attributes price of each group when on
  function zen_get_products_base_price_w($products_id) {
    global $db;
      $product_check = $db->Execute("select products_price_w, products_priced_by_attribute from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");

// is there a products_price to add to attributes
      $products_price_w = $product_check->fields['products_price_w'];
...
// add attributes price to price
      if ($product_check->fields['products_priced_by_attribute'] == '1' and $product_att_query->RecordCount() >= 1) {
        ...
        $the_base_price_w = $products_price_w + $the_base_price;
      } else {
        $the_base_price_w = $products_price_w;
      }
